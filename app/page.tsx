...
export default function Dashboard() {
  ...
  // Fungsi untuk mengeksekusi POST request ke API Anda
              ...
              <tbody>
                {wallets.length === 0 ? (
                  ...
                  wallets.map((wallet: any, index: number) => (
                    <tr key={index} className="border-b border-gray-750 hover:bg-gray-750">
                      <td className="py-4 font-mono text-sm">{wallet.wallet_address}</td>
                      <td className="py-4 capitalize">{wallet.chain_network}</td>
                      <td className="py-4 text-green-400 text-sm">Terlacak</td>
                    </tr>
                  ))
                )}
              </tbody>
